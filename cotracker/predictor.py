    ...
    def _compute_sparse_tracks(
        ...
        if support_grid_size > 0:
            grid_pts = get_points_on_a_grid(
                support_grid_size, self.interp_shape, device=video.device
            )
        ...
        if backward_tracking:
            ...
            if support_grid_size > 0:
                queries[:, -support_grid_size**2 :, 0] = T - 1
        if support_grid_size > 0 and not keep_grid:
            tracks = tracks[:, :, : -support_grid_size**2]
            visibilities = visibilities[:, :, : -support_grid_size**2]
        thr = 0.9
...
class CoTrackerOnlinePredictor(torch.nn.Module):
    ...
    def forward(
        ...
        if is_first_step:
            ...
            if add_support_grid:
                grid_pts = get_points_on_a_grid(
                    support_grid_size, self.interp_shape, device=video_chunk.device
                )
                grid_pts = torch.cat([torch.zeros_like(grid_pts[:, :, :1]), grid_pts], dim=2)
                queries = torch.cat([queries, grid_pts], dim=1)
