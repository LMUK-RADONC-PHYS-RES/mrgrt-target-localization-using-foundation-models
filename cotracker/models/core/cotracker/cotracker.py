...
class CoTracker2(nn.Module):
    ...
    def forward(self, video, queries, iters=4, is_train=False, is_online=False):
        ...
        for ind in indices:
            ...
            if True or ind == 0:
                ...
                coords_predicted[:, ind+step : ind + S] = coords[-1][:, step:S_trimmed]
                vis_predicted[:, ind +step: ind + S] = vis[:, step:S_trimmed]
            if is_train:
    ...
    def forward_online(self, video, queries, iters=4, is_train=False, is_online=False):
        """Predict tracks
        ...
        if self.online_coords_predicted is None:
            ...
            # -> taget length ?
            pad = min(step, T - step)
            #print(f"Prediction pad: {pad}, T = {T}, step = {step}")
            #print(f"Prediction shape: {self.online_coords_predicted.shape}")
            coords_predicted = F.pad(
                self.online_coords_predicted, (0, 0, 0, 0, 0, pad), "constant"
            )
            vis_predicted = F.pad(self.online_vis_predicted, (0, 0, 0, pad), "constant")
        ...
        if True: # all good
            ...
            # TODO: we may drop this requirement because the transformer should not care
            # TODO: pad the features instead of the video
            pad = S - T # We don't want to pad if T % S == 0
        ...
        if self.online_track_feat is None:
            self.online_track_feat = torch.zeros_like(track_feat, device=device)
            self.online_track_feat += track_feat
        
        if False: # update track features for the current window
            # set online_track_feat to the value of the query
            # for online this is always 0, since only the last query is used 
            # ignore for now
            sample_frames = queried_frames[:, None, :, None]  # B 1 N 1
            left = self.online_ind + (step if self.online_ind == 0 else 0)
            right = self.online_ind + S # update to match step
            #right = self.online_ind + T 
            sample_mask = (sample_frames >= left) & (sample_frames < right)
            print(f"sample_frames {sample_frames.flatten().cpu().numpy().astype(int)}")
            print(f"TrackFeatures: left: {left}, right: {right}, sample_mask: {sample_mask.flatten().cpu().numpy().astype(int)}")  
            print(f"sample_mask: {sample_mask.shape}")  
            self.online_track_feat += track_feat * sample_mask
            track_feat = self.online_track_feat.clone()
        ...
        ind = self.online_ind
        #for ind in indices: # no iteration
        if True: # no iteration
            # We copy over coords and vis for tracks that are queried
            # by the end of the previous window, which is ind + overlap
            if ind > 0:
                overlap = S - step
                # -> copy over the last step of the previous window
                # ->
                coords_init = torch.nn.functional.pad(
                    coords_predicted[:, ind : ind + overlap] / self.stride,
                    (0, 0, 0, 0, 0, step),
                    "replicate",
                )  # B S N 2
                vis_init = torch.nn.functional.pad(
                    vis_predicted[:, ind : ind + overlap, :, None].clone(),
                    (0, 0, 0, 0, 0, step),
                    "replicate",
                )  # B S N 1
    ...
    def forward_offline(self, video, queries, iters=4, is_train=False, is_online=False):
        """Predict tracks
